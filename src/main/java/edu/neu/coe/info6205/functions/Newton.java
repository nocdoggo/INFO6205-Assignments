...
public class Newton {
    ...
    public static void main(String[] args) {
        ...
        // Build the Newton's Approximation problem to be solved: sin(x)*cos(x) = x
        // Per the math, we first modify the function to make it align with the expression as:
        //          sin(x)*cos(x) - x = 0
        // Then, we take the derivative, and then we yield:
        //          cos(x)*cos(x) + sin(x)*[-sin(x)] - 1 = 0
        //      ==> cos(x)*cos(x) - sin(x)*sin(x) - 1 = 0
        Newton newton = new Newton("sin(x)*cos(x) = x", (double x) -> Math.sin(x) * Math.cos(x) - x, (double x) -> Math.cos(x) * Math.cos(x) - Math.sin(x) * Math.sin(x) - 1);
